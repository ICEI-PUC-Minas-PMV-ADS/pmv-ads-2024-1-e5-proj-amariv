...
export function RegisterPage() {
  ...
  const handleSubmit = React.useCallback(async (evt: React.FormEvent<HTMLFormElement>) => {
    try {
      evt.preventDefault();

      const phoneIsInvalid = !MaskedUtils.isValidPhone(phone) && !MaskedUtils.isValidMobile(phone);
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Input
            ...
            label='Confirmar senha'
             onChange={(evt) => { setConfPassword(evt.target.value) }}
             value={confPassword}
             required
             minLength={6} />
          <Input
